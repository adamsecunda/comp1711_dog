...
int main()
{
    ...
    while (1)
    {
        ...
        case 'f':
            ...
            {
                tokeniseRecord(line, ",", daily_readings[counter].date, &daily_readings[counter].bloodIron);
                float iron = daily_readings[counter].bloodIron;
                if(iron < min || min == 0){
                    min = iron;
                }else if(iron > max){
                    max = iron;
            ...
            float range = max - min;
            printf("Your blood iron range was %.2f\n", range);
            fclose(input);
            break;
        case 'G':
        case 'g':
            ...
            while (fgets(line, buffer_size, input))
            {
                tokeniseRecord(line, ",", daily_readings[counter].date, &daily_readings[counter].bloodIron);
                float iron = daily_readings[counter].bloodIron;
                for(int i = 0; i < (int) iron; i++){
                    //Add all the readings to a list
                    //Print to text file in the format for chart.js
                }
                printf("\n");
                counter++;
            }
